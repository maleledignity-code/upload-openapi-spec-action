...
import { APIPromise } from 'hh/core/api-promise';

import util from 'node:util';
import Hh from 'hh';
import { APIUserAbortError } from 'hh';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    const forceAPIResponseForClient = async (client: Hh) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Hh({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('default logLevel is warn', async () => {
      const client = new Hh({ apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Hh({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['HH_LOG'] = 'debug';
      const client = new Hh({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['HH_LOG'] = 'not a log level';
      const client = new Hh({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'HH_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['HH_LOG'] = 'debug';
      const client = new Hh({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  test('explicit global fetch', async () => {
    ...
    const client = new Hh({ baseURL: 'http://localhost:5000/', apiKey: 'My API Key', fetch: defaultFetch });
  });
  ...
  test('normalized method', async () => {
    ...
    const client = new Hh({ baseURL: 'http://localhost:5000/', apiKey: 'My API Key', fetch: testFetch });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Hh({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['HH_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Hh({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['HH_BASE_URL'] = 'https://example.com/from_env';
      const client = new Hh({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['HH_BASE_URL'] = ''; // empty
      const client = new Hh({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });

    test('blank env variable', () => {
      process.env['HH_BASE_URL'] = '  '; // blank
      const client = new Hh({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Hh({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['HH_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Hh({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Hh({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Hh({ apiKey: 'My API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  describe('withOptions', () => {
    test('creates a new client with overridden options', async () => {
      const client = new Hh({ baseURL: 'http://localhost:5000/', maxRetries: 3, apiKey: 'My API Key' });
    ...
    test('respects runtime property changes when creating new client', () => {
      const client = new Hh({ baseURL: 'http://localhost:5000/', timeout: 1000, apiKey: 'My API Key' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Hh({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Hh({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Hh({ apiKey: 'My API Key', fetch: testFetch });
