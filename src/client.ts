...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['HH_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['HH_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Hh API.
 */
export class Hh {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['HH_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('HH_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.HhError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Hh client with an apiKey option, like new Hh({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Hh.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('HH_LOG'), "process.env['HH_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('HH_CUSTOM_HEADERS');
    if (customHeadersEnv) {
